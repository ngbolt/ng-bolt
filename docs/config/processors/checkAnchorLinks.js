...
module.exports = function checkAnchorLinksProcessor(log, resolveUrl, extractLinks) {
  return {
    ...
    $process: function(docs) {
      ...
      // console.log(allValidReferences);
      ...
      _.forEach(allDocs, function(linkInfo) {
        ...
        _(linkInfo.hrefs)
          ...
          .forEach(function(link) {
            // Add webroot to link in docs for resolveUrl to work correctly
            link = path.join(webRoot, link);
            var baseLink = path.join(webRoot, linkInfo.path, link);
            ...
            if ( !_.some(pathVariants, function(pathVariant) {
              ...
              var normalizedBaseLink = path.join(webRoot, resolveUrl(linkInfo.path, baseLink, base));
              if ( !_.some(pathVariants, function(pathVariant) {
                return allValidReferences[decodeURIComponent(normalizedBaseLink + pathVariant)];
              }) ) {
