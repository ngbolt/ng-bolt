...
function buildDocData(doc) {
  return _.assign({
    ...
    path: doc.path && doc.path.endsWith('home') ? '' : doc.path,
    order: parseInt(doc.sortorder, 10) || 100
  });
}

module.exports = function homePagesProcessor(moduleMap) {
  return {
  ...
  function process(docs) {
    ...
    var homepages = _(docs).filter(function (doc) {
      return doc.docType === 'content' && doc.module === 'home';
    })
